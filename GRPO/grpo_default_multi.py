...
from utils import compute_metrics, convert_to_builtin_types, preprocess_default
...
if __name__ == "__main__":
    ...
    model_paths = [
        'default-0.5016-1854954-mms-0.2.pt',
        'default-0.5224-28734958-mms-0.2.pt',
        'default-0.5196-38985923-mms-0.2.pt',
        'default-0.5167-709090234-mms-0.2.pt',#0.05056887865066528
        'default-0.5100-832748293-mms-0.2.pt',
        'default-0.5009-41281931-mms-0.2.pt',
        'default-0.5020-611129304-mms-0.2.pt',
        'default-0.5097-55232342-mms-0.2.pt',
        'default-0.5223-98293842-mms-0.2.pt',
        'default-0.5065-85902025-mms-0.2.pt',
    ]
    for model_path in model_paths:
        ...
        _, X_val, _, y_val, sens_idx = preprocess_default(seed)
        sens_val = X_val[:, sens_idx]
        sens_classes = [0, 1]

        # X_val, y_val, sens_val = X_val.to(device), y_val.to(device), sens_val.to(device)

        model = DefaultModel(p=p).to(device)
        state_dict = torch.load(f"../saved_models/default_multi/{model_path}", map_location=torch.device('cpu'))
        ...
        save_dir = f"intermediate results/Default_multi"
