...
from utils import compute_metrics, convert_to_builtin_types, preprocess_meps16_tutorial
...
def train_grpo(env, max_episodes=500, batch_size=10, lr=0.002, n_iterations=10, eps=0.2):
    """
    使用GRPO训练神经元缩放环境
    ...
    return env.best_fairness, env.best_performance, env.best_scales
...
if __name__ == "__main__":
    hidden_dims = [256, 256, 64, 64, 32, 32, 16, 8]
    key_layers_num = 2
    threshold = 0.01
    max_episodes = 500
    ...
    model_paths = [
        'meps16-0.5322-29845902-mms-0.5.pt',
        'meps16-0.5162-94572893-mms-0.5.pt',
        'meps16-0.5328-73894923-mms-0.5.pt',
        'meps16-0.5293-62832934-mms-0.5.pt',
        'meps16-0.5178-14374933-mms-0.5.pt',
        'meps16-0.5310-3182394-mms-0.5.pt',
        'meps16-0.5162-4287385-mms-0.5.pt',
        'meps16-0.5480-412673726-mms-0.5.pt',
        'meps16-0.5316-57843267-mms-0.5.pt',
        'meps16-0.5231-6892903-mms-0.5.pt',
    ]
    sens_classes_dict = {
        29845902: [-0.7876802087,1.2695508003],
        94572893: [-0.7888960242,1.2675942183],
        73894923: [-0.7896564007,1.2663735151],
        62832934: [-0.7910261154,1.2641807795],
        14374933: [-0.7890480757,1.2673499584],
        3182394: [-0.7927019596,1.2615082264],
        4287385: [-0.7860100865,1.2722482681],
        412673726: [-0.7910261154,1.2641807795],
        57843267: [-0.7829782963,1.2771745920],
        6892903: [-0.7869208455,1.2707759142]
    }
    for model_path in model_paths:
        ...
        _, X_val, _, y_val, sens_idx = preprocess_meps16_tutorial(seed)
        ...
        state_dict = torch.load(f"../saved_models/meps16_multi/{model_path}", map_location=torch.device('cpu'))
        model.load_state_dict(state_dict)
        # model.load_state_dict(torch.load(f"../saved_models/adult/{model_path}"))
        model.eval()

        # baseline_eod, baseline_f1, baseline_acc = compute_metrics(
        ...
        layer_sizes_accuracy, key_neurons, total_train_time = (
           ...
           sens_idx, threshold, hyperparameter_search_type, "meps16", sens_classes).identify())
        ...
        save_dir = f"intermediate results/Meps16_multi"
        os.makedirs(save_dir, exist_ok=True)

        save_path = os.path.join(save_dir, f"{seed}_{max_episodes}_{threshold}_{layers_mode}_{neurons_mode}_{hyperparameter_search_type}_cv5.json")
        ...
        best_fairness, best_performance, best_scales = train_grpo(
            env=env,
