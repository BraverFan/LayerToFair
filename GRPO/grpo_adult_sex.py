...
def train_grpo(env, max_episodes=500, batch_size=10, lr=0.002, n_iterations=10, eps=0.2):
    ...
    print(history["best_fairness"])
    print(len(history["best_fairness"]))

    return env.best_fairness, env.best_performance, env.best_scales, history["best_fairness"]
...
if __name__ == "__main__":
    ...
    for model_path in model_paths:
        ...
        save_dir = f"Ablation experiment/Adult_sex_key_full_layer"
        os.makedirs(save_dir, exist_ok=True)

        save_path = os.path.join(save_dir, f"{seed}_{max_episodes}_{layers_mode}_{neurons_mode}.json")
        ...
        best_fairness, best_performance, best_scales, best_fairness_list = train_grpo(
            env=env,
        ...
        save_dict["best_fairness_list"] = [float(x) for x in best_fairness_list]
