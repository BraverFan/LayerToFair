...
if __name__ == "__main__":
    ...
    model_paths = [
        'meps16-0.5047-42-mms-0.1.pt',
       'meps16-0.5364-2579600519-mms-0.1.pt',
        'meps16-0.5023-608130088-mms-0.1.pt',
        'meps16-0.5166-2123741781-mms-0.1.pt',
        'meps16-0.5256-498349121-mms-0.1.pt',
        'meps16-0.5261-523624893-mms-0.1.pt',
        'meps16-0.5056-982938842-mms-0.1.pt',
        'meps16-0.5116-56738471-mms-0.1.pt',
        'meps16-0.5172-44442930-mms-0.1.pt',
        'meps16-0.5035-14859343-mms-0.1.pt',
    ]
    sens_classes_dict = {
        42: [-7.8585833311e-01, 1.2724939585e+00],
        398198020: [-0.7873764038, 1.2700406313],
        36821596: [-0.7864654064, 1.2715117931],
        2579600519: [-0.7875282764, 1.2697956562],
        3569605705: [-0.7887439728, 1.2678384781],
        608130088: [-0.7890480757,1.2673499584],
        2123741781: [-0.7939219475,1.2595696449],
        498349121: [-0.7931593060,1.2607806921],
        523624893: [-0.7969763875,1.2547422647],
        982938842: [-0.7923970819,1.2619935274],
        56738471: [-0.7881360054,1.2688164711],
        44442930: [-0.7930068374,1.2610231638],
        14859343: [-0.7920923233,1.2624790668]
    }
    for model_path in model_paths:
        ...
        # num_1 = (sens_val == -7.8585833311e-01).sum().item()
        # num_0 = (sens_val == 1.2724939585e+00).sum().item()
        # total = sens_val.numel()  # 等价于 len(sens_val)
        #
        # # 计算比例
        # ratio_1 = num_1 / total
        # ratio_0 = num_0 / total
        #
        # print(f"🔢 sens_val = 1: {num_1} ({ratio_1:.4f})")
        # print(f"🔢 sens_val = 0: {num_0} ({ratio_0:.4f})")
        ...
        save_dir = f"intermediate results/Meps16_time"
        os.makedirs(save_dir, exist_ok=True)

        save_path = os.path.join(save_dir, f"{seed}_{layers_mode}_{neurons_mode}_{max_time_minutes}_0.001threshold.json")
